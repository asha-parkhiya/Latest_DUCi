...
import com.sparkle.devicescanner.BuildConfig;
...
import static com.sparkle.devicescanner.Utils.CrcCalculator.TestBytes;
...
public class ConstantMethod {
    ...
    private static String[] hash_low_high = new String[2];
    ...
    //    String[] hash_low_high = new String[2];
    public static String[] extract_hash_low_high(String formatted_hash_str){


        // strip out spaces
//        console.log('String received'); // '*011 207 593 220 164 276 300#'
        formatted_hash_str = formatted_hash_str.replace(" ", "");
        formatted_hash_str = formatted_hash_str.replace("*","");
        formatted_hash_str = formatted_hash_str.replace("#","");

        formatted_hash_str = formatted_hash_str.substring(1);
//        System.out.println("---------remove--------------"+formatted_hash_str);
        hash_low_high[0] = formatted_hash_str.substring(0, 10);
        hash_low_high[1] = formatted_hash_str.substring(10,20);
//        System.out.println("---------------------"+hash_low_high[0]);
//        System.out.println("---------------------"+hash_low_high[1]);

        return hash_low_high;
    }

    public static String code_to_hash_str(String formatted_hash_str){

        hash_low_high = extract_hash_low_high(formatted_hash_str);
        String h = Integer.toHexString((int) Long.parseLong(hash_low_high[0]));
        String h1 = Integer.toHexString((int) Long.parseLong(hash_low_high[1]));
        String hash_str = h + h1;
    ...
    public static String code_has_for_lumn(String formatted_hash_str){
        formatted_hash_str = formatted_hash_str.replace(" ", "");
        formatted_hash_str = formatted_hash_str.replace("*","");
        formatted_hash_str = formatted_hash_str.replace("#","");
        formatted_hash_str = ">WOTP:"+formatted_hash_str+"<";
        return formatted_hash_str;
    }

    public static String write_code_for_ovCamp(String formatted_hash_str,String PPID, String cmdLength){
        hash_low_high = extract_hash_low_high(formatted_hash_str);
        String h = Integer.toHexString((int) Long.parseLong(hash_low_high[0]));
        String h1 = Integer.toHexString((int) Long.parseLong(hash_low_high[1]));
        String hashtop = h + h1;
        hashtop = hashtop.toUpperCase();
        ...
        String hash = makeHastop(hashtop);
//        System.out.println(PPID);

        String makedata = "00000000000000000000000000000000C56A29"+ cmdLength+ hash;
        String finaldata = makedata;
        makedata = makedata.replaceAll("..", "$0 ").trim();
        byte[] datatmp = ConstantMethod.hexStr2Bytes(makedata);

        String CRC = CheckCRC_8_MAXIM(Crc8.Params, datatmp);
        if (CRC.length() == 1) {
            CRC = "0" + CRC;
        }
        finaldata = finaldata + CRC;
        finaldata = finaldata.replaceAll("..", "$0 ").trim();

        return finaldata;
    }

    public static String code_string_for_card(String formatted_hash_str,String PPID){
        hash_low_high = extract_hash_low_high(formatted_hash_str);
        String h = Integer.toHexString((int) Long.parseLong(hash_low_high[0]));
        String h1 = Integer.toHexString((int) Long.parseLong(hash_low_high[1]));
        String hashtop = h + h1;
...
//        System.out.println(PPID);
        ...
        String makedata = "00000000000000000000000000000000C56A290EFA" + hash;

//        String headerlen = FindLength("C56A2901033A3A3A000100003A3A3A" + ConstantMethod.ASCIItoHEX(ascii) + "3A3A3A" + hash);// this only for card read write.
//        String makedata = "00000000000000000000000000000000C56A29" + headerlen + "01033A3A3A000100003A3A3A" + ConstantMethod.ASCIItoHEX(ascii) + "3A3A3A" + hash;
        String finaldata = makedata;
        makedata = makedata.replaceAll("..", "$0 ").trim();
        byte[] datatmp = ConstantMethod.hexStr2Bytes(makedata);
...
//        byte[] datatmp1 = ConstantMethod.hexStr2Bytes(finaldata);
//        for (int i = 0; i < datatmp1.length; i++) {
//            System.out.print("========"+datatmp1[i]);
//        }

        return finaldata;
    }

    public static String Cal_CRC_8(AlgoParams[] params, String string) {
        string = string.replaceAll("..", "$0 ").trim();
        byte[] datatmp = ConstantMethod.hexStr2Bytes(string);
        for (int i = 0; i < params.length; i++) {
            CrcCalculator calculator = new CrcCalculator(params[i]);
            long result = calculator.Calc(datatmp, 0, datatmp.length);
    ...
    public static String makeHastop(String hashtop) {
...
//        System.out.println("------------");
    ...
    public static String FindLength(String data) {
        int len = data.length();
        len = len / 2;
        len = len + 2;
    ...
    public static String CheckCRC_8_MAXIM(AlgoParams[] params, byte[] TestBytes) {
        for (int i = 0; i < params.length; i++) {
            CrcCalculator calculator = new CrcCalculator(params[i]);
...
//                System.out.println(calculator.Parameters.Name + " - BAD ALGO!!! " + Long.toHexString(result).toUpperCase());
    ...
    public static String convertHextoDecimal(String hex) {
        ...
        String i = String.valueOf(Integer.parseInt(hex, 16));
