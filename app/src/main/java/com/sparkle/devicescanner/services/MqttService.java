...
//            Toast.makeText(this, "subscribe: "+topic, Toast.LENGTH_SHORT).show();
    ...
    public class MyMqttCallback implements MqttCallback {
        ...
        public void messageArrived(String topic, MqttMessage message) {
            String devicetype = "";
            String ppid = "";
            System.out.println("------------arrived---------"+"Message arrived: " + topic + ":" + message.toString());

            topic = topic.replace("_OV1/","");
            if (topic.startsWith("ovCamp")){
                devicetype = "ovCamp";
                topic = topic.replace("ovCamp/","");
            }else {
                devicetype = "LUMN";
                topic = topic.replace("LUMN/","");
            }
            ppid = topic;

            String codehasformat = message.toString();
//            EventBus.getDefault().postSticky(new CodeStringEvent(devicetype,ppid,codehasformat));
            EventBus.getDefault().post(new CodeStringEvent(devicetype,ppid,codehasformat));
//            String str = message.toString();
//            String PPID = null,CODE = null,TYPE = null;
//            String[] separated = str.split("PPID:");
//            String[] separated1 = separated[1].split(" CODE:");
//            System.out.println("---------ppid----------"+separated1[0]);
//            String[] separated2 = separated1[1].split(" TYPE:");
//            System.out.println("---------code----------"+separated2[0]);
//            System.out.println("---------type----------"+separated2[1]);
//            PPID = separated1[0];
//            CODE = separated2[0];
//            TYPE = separated2[1];
////            EventBus.getDefault().post(new ToDo(PPID,CODE,TYPE));
//            newAdapter.filter(PPID);
//            string = myPref.getPref("string","");
//            position = myPref.getPref("position",0);
//            if (string.contains(PPID)){
//                toDoList.remove(position-1);
//                toDoList.add(position-1,new ToDo(PPID,CODE,TYPE));
//                myPref.setPref("string","");
//                myPref.setPref("position",0);
//            }else {
//                toDoList.add(new ToDo(PPID,CODE,TYPE));
//            }
//            myPref.setPref3("todolist",toDoList);
        }
