...
import com.sparkle.devicescanner.events.UsbSerialActionEvent;
...
public class UsbService extends Service {
    ...
    private boolean LUMN_HAND = false;
...
//                EventBus.getDefault().post("Usb on Receive data : "+myPref.getPref(Constant.LUMN_USB,true));
//                System.out.println("--------lumn usb------------"+myPref.getPref(Constant.LUMN_USB,true));
                if (!myPref.getPref(Constant.LUMN_USB,false)){
                    String message1 = ConstantMethod.bytes2Str(arg0);
//                    System.out.println("message from lumn: " + message1);
                    EventBus.getDefault().post("LUMN_USB");
                    EventBus.getDefault().post(new UsbSerialMessageEvent(MESSAGE_FROM_SERIAL_PORT, message1));
                }else{
                    String message = ConstantMethod.bytes2HexStr(arg0);
//                    System.out.println("message from camp: " + message);
                    EventBus.getDefault().post("OVCAMP_USB");
                    EventBus.getDefault().post(new UsbSerialMessageEvent(MESSAGE_FROM_SERIAL_PORT, message));
                }
//                if (!myPref.getPref(Constant.OVCAMP_USB,false)){
//                    String message = ConstantMethod.bytes2HexStr(arg0);
//                    System.out.println("message from card: " + message);
//                    EventBus.getDefault().post("OVCAMP_USB");
//                    EventBus.getDefault().post(new UsbSerialMessageEvent(MESSAGE_FROM_SERIAL_PORT, message));
//                }
    ...
    public void write(byte[] data) {
        if (serialPort != null){
//            System.out.println("----------lumn usb----------"+myPref.getPref(Constant.LUMN_USB,false));
            if (!myPref.getPref(Constant.LUMN_USB,false)){
//                System.out.println("--------write data--lumn--------"+data);
                baudrate9600(data);
            }else {
//                EventBus.getDefault().post("-----ovcamp data write-----");
//                System.out.println("--------write data---ovcamp-------"+data);
                baudrate38400(data);
            }
//            if (!myPref.getPref(Constant.OVCAMP_USB,false)){
//                EventBus.getDefault().post("Baudrate : 38400");
//                baudrate38400(data);
//            }
        }
    }
